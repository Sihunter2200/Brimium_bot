...
async def send_visualization(bot, chat_id, room_path, material_path, user_id, bg, try_another, leave_request, state, type_layout):
    photo_layout=LAYOUTS[type_layout]['photo']
    layout_prompt=LAYOUTS[type_layout]['prompt']
    try:
        url, error = await image_api.visualize(room_path, material_path, photo_layout, layout_prompt)
    except Exception:
...
async def save_photo(message: Message, widget: MessageInput, dialog_manager: DialogManager):
    ...
    material_id = dialog_manager.start_data.get('material_id') # type: ignore
    if material_id is None:
        await message.answer(i18n.stop.choice.material())
        return

    async with async_session() as session:
        path_mat = await requests.get_photo_by_material_id(session, int(material_id))
        material_name = await requests.get_material_name(session, int(material_id)) # type: ignore

    type_layout=dialog_manager.start_data.get('type_layout')
    brick_type = dialog_manager.start_data.get('brick_type')

    state = dialog_manager.middleware_data['state']
    await state.update_data(material_id=dialog_manager.start_data.get('material_id'), material_name=material_name, \
                            user_photo_path=path_user, username=message.from_user.username,\
                            type_layout=type_layout, brick_type=brick_type)

    bg = dialog_manager.bg(user_id=user_id, chat_id=message.chat.id)
    asyncio.create_task(send_visualization(message.bot, message.chat.id, path_user, path_mat, user_id, bg, i18n.to.choice.material(), i18n.leave.request(), state, type_layout))
